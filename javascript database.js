class Database {
  ...
  getVotingStatus(username) {
    return {
      hasVoted: username ? !!this.votes.voters[username] : false,
      votedFor: username ? this.votes.voters[username] : null,
      counts: {
        assets: this.votes.assets,
        themes: this.votes.themes,
        logos: this.votes.logos
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const updateVotingUI = async () => {
    const username = (await window.websim.getUser())?.username;
    const votingStatus = database.getVotingStatus(username);
    const votingEnd = new Date('2025-06-01T23:59:59Z');
    const now = new Date();
    ...
    if (now > votingEnd) {
      // Show voting results
      const winner = Object.entries(votingStatus.counts)
        .reduce((a, b) => a[1] > b[1] ? a : b)[0];
      
      votingContent.innerHTML = `
        <div class="voting-ended">
          <h2>Voting Has Ended</h2>
          <p class="winner">"${winner.charAt(0).toUpperCase() + winner.slice(1)}" won the vote!</p>
          <p class="implementation-note">The winning feature will be implemented soon. Thank you for participating!</p>
        </div>
      `;
    } else {
      // Update progress bars and vote counts
      const total = Object.values(votingStatus.counts).reduce((a, b) => a + b, 0);
      
      Object.entries(votingStatus.counts).forEach(([option, count]) => {
        const progress = document.getElementById(`${option}-progress`);
      ...
      // Update voting buttons status
      document.querySelectorAll('.vote-btn').forEach(btn => {
        if (votingStatus.hasVoted) {
          btn.disabled = true;
          if (votingStatus.votedFor === btn.dataset.option) {
  ...
  document.querySelectorAll('.vote-btn').forEach(btn => {
    btn.addEventListener('click', async () => {
      try {
        const username = (await window.websim.getUser())?.username;
        if (!username) {
          throw new Error('Please wait while we load your user data');
        }

        const option = btn.dataset.option;
        await database.castVote(option, username);
        updateVotingUI();
      } catch (error) {
        alert(error.message);
      }
